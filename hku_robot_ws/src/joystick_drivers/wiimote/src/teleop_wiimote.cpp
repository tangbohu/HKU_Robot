...
while(abs(last_y)>0.01||abs(last_x)>0.01||abs(last_z)>0.01)
{

    vel.linear.x =0.8*last_x;
    vel.linear.y =0.8*last_y;
    vel.linear.z =0.8*last_z;
  ...
  {
    if (njoy_in_use_||last_x>abs_error||last_y>abs_error||last_z>abs_error)
    {
      vel_pub_.publish(vel);
...
while(abs(last_y)>0.01||abs(last_x)>0.01||abs(last_z)>0.01)
{
  ...
  else
  {
    ...
    if (dpad_in_use_||last_y>abs_error||last_x>abs_error||last_x>abs_error)
    {
      vel_pub_.publish(vel);
      dpad_in_use_ = false;
    }
