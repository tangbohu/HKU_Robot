...
void callback( const sensor_msgs::ImageConstPtr& msg_rgb , const sensor_msgs::ImageConstPtr& msg_depth )
{
    ...
    cv::Mat& mat_depth = img_ptr_depth->image;
        ...
        sprintf( file_rgb, "/%06d_rgb.png", cnt );
        sprintf( file_depth, "/%06d_depth.png", cnt );
		
        std::string const& file_rgb_string = filepath+ std::string(file_rgb);
        sprintf( file_rgb, file_rgb_string.c_str());

        std::string const& file_depth_string = filepath+ std::string(file_depth);
        sprintf( file_depth, file_depth_string.c_str());
...
int main(int argc, char** argv)
{
  ...
  filepath =(argv[1]);
    ...
    message_filters::Subscriber<sensor_msgs::Image> subscriber_depth( nh , "/camera/depth_registered/hw_registered/image_rect" , 1 );
  ...
  sync.registerCallback(boost::bind(&callback, _1, _2));
	...
	while(ros::ok())
	{
        ...
        ROS_INFO_STREAM(
                        "Enter 'a' to save a pair of images or 'b' to automatically save all images\n");
		cin.get(c);
		cin.ignore();
		c = tolower(c);
