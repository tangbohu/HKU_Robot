...
public:
	...
	//-------------------------------------------------------------//
	// Add by Prashant
	template< class T1> Vec<D,T> &operator = (T1 val)
	{ for (int i = 0; i < D; i++) v[i] = val;  return *this; } 
	//-------------------------------------------------------------//
...
typedef Vec<3,int> Vector3i;     // Add by Song Peng
typedef Vec<4, int> Vector4i;
typedef Vec<2,float> Vector2f;   // Add by Philip
...
static inline const Vec<3, T> operator | (const Vec<3, T> &v1, const Vec<3, T> &v2)
{
	return Vec<3, T>(std::min(v1[0],v2[0]),
		std::min(v1[1], v2[1]),
		std::min(v1[2], v2[2]));
}
#define MINVEC |
...
static inline const Vec<3, T> operator && (const Vec<3, T> &v1, const Vec<3, T> &v2)
{
	return Vec<3, T>(std::max(v1[0], v2[0]),
		std::max(v1[1], v2[1]),
		std::max(v1[2], v2[2]));
}
#define MAXVEC &&
...
struct Vec3iComp
{
	...
	bool operator () (const Vector3i & ls, const Vector3i &rs) const
	{
		return (ls[0] < rs[0] || (ls[0] == rs[0] && ls[1] < rs[1]) || (ls[0] == rs[0] && ls[1] == rs[1] && ls[2] < rs[2]));
	}

};


#define voxel_X_move Vector3i(1,0,0)
#define voxel_Y_move Vector3i(0,1,0)
#define voxel_Z_move Vector3i(0,0,1)
